...
if (!config.token) {
  logError();
  return console.log(red(`                  Token du bot manquant dans`), magenta(`config.json`), red(`!\n\n`));
}
if (!config.id.ping_test || !config.id.server || !config.id.your_id) {
  logError();
  return console.log(red(`                  IDs manquants dans`), magenta(`config.json`), red(`!\n\n`));
}
...
client.on("ready", () => {
  ...
  if (config.activity) {
    console.log(green(`CLIENT | Activité :`), magenta(`${config.activity}`), green("."));
  } else {
    console.log(green(`CLIENT | Activité :`), yellowBright(`/`), green("."));
  }
});
...
client.on("messageCreate", (msg) => {
  ...
  client.guilds.cache.get(config.id.server).members.cache.forEach((mbmr) => {
    ...
    mbmr.send(msg_to_send).then((sended) => {
      console.log(green(`DM | DM`), magenta(msg_to_send), green(`envoyé à`), magenta(`${mbmr.user.username} (${mbmr.user.id})`), green("!"));
    });
  });
